...
const BoutiqueNigerianPage = () => {
  ...
  const nigerianCategories = ['Traditional', 'Senator Wear', 'Agbada', 'Kaftan', 'Bubu', 'Ankara', 'Dashiki'];
  ...
  const loadProducts = async () => {
    try {
      ...
      // Filter for Nigerian traditional wear
      const nigerianProducts = response.data.filter(p => 
        nigerianCategories.includes(p.category)
      );
      setProducts(nigerianProducts);
    } catch (error) {
    ...
    <div className="min-h-screen py-16 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="font-oswald text-5xl font-bold uppercase" data-testid="boutique-title">Nigerian Traditional Wear</h1>
            <p className="font-manrope text-lg text-zinc-600 mt-2">
              Authentic Nigerian clothing - Agbada, Senator wear, and more
            </p>
          </div>
        ...
        {products.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-zinc-600">No Nigerian traditional wear available at the moment.</p>
          </div>
        ) : (
...
export default BoutiqueNigerianPage;
